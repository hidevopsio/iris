...
### Looking for free and real-time support?
    ...
    https://github.com/hidevopsio/iris/issues
    https://chat.iris-go.com

### Looking for previous versions?

    https://github.com/hidevopsio/iris/releases
...
**How to upgrade**: Open your command-line and execute this command: `go get -u github.com/hidevopsio/iris` or let the automatic updater do that for you.

# Tu, 30 October 2018 | v11.0.2

Fix [memstore](core/memstore/memstore.go) overflows when build 32 bit app, reported and fixed by [@bouroo](https://github.com/bouroo) at: https://github.com/hidevopsio/iris/issues/1118

# Su, 28 October 2018 | v11.0.1

- Update benchmarks: https://github.com/hidevopsio/iris/commit/d1b47b1ec65ae77a2ca7485e510386f4a5456ac4
- Add link for third-party source benchmarks: https://github.com/hidevopsio/iris/commit/64e80a7ee5c23ed938ddc8b68d181a25420c7653
- Add optionally custom low-level websocket message data prefix as requested at: https://github.com/hidevopsio/iris/issues/1113 by [@jjhesk](https://github.com/jjhesk). Example:
...
For the craziest of us, click [here](https://github.com/hidevopsio/iris/compare/v10.7.0...v11) 🔥 to find out the commits and the code changes since our previous release.
...
- The `Context#ReadForm` will no longer return an error if it has no value to read from the request, we let those checks to the caller and validators as requested at: https://github.com/hidevopsio/iris/issues/1095 by [@haritsfahreza](https://github.com/haritsfahreza)
...
The `github.com/hidevopsio/iris/core/router.AllMethods` is now a variable that can be altered by end-developers, so things like `app.Any` can register to custom methods as well, as requested at: https://github.com/hidevopsio/iris/issues/1102. For example, import that package and do `router.AllMethods = append(router.AllMethods, "LINK")` in your `main` or `init` function.

The old `github.com/hidevopsio/iris/core/router/macro` package was moved to `guthub.com/kataras/iris/macro` to allow end-developers to add custom parameter types and macros, it supports all go standard types by default as you will see below.
...
The [vendor](https://github.com/hidevopsio/iris/tree/master/vendor) folder will be kept until the majority of Go developers get acquainted with the new `go modules`.  The `go.mod` and `go.sum` files will come at `iris v12` (or `go 1.12`), we could do that on this version as well but I don't want to have half-things, versioning should be passed on import path as well and that is a large breaking change to go with it right now, so it will probably have a new path such as `github.com/hidevopsio/iris/v12` based on a `git tag` like every Iris release (we are lucky here because we used semantic versioning from day zero). No folder re-structure inside the root git repository to split versions will ever happen, so backwards-compatibility for older go versions(before go 1.9.3) and iris versions will be not enabled by-default although it's easy for anyone to grab any version from older [releases](https://github.com/hidevopsio/iris/releases) or branch and target that.
...
Version 10.7.0 is part of the official [releases](https://github.com/hidevopsio/iris/releases).
...
- Change connection list from a customized slice to `sync.Map` with: [this](https://github.com/hidevopsio/iris/commit/5f16704f45bedd767527eadf411cf9bc0f8edaee) and [that commit](https://github.com/hidevopsio/iris/commit/16b30e8eed1406c61abc01282120870bd9fa31d8)
- Minify and add the `iris-ws.js` to the famous https://cdnjs.com via [this PR](https://github.com/hidevopsio/iris/pull/1053) made by [Dibyendu Das](https://github.com/dibyendu)
...
- Add `OnConnectionClose` function listener to fire a callback when the underline tcp connection is closed, extremely useful for SSE or other loop-forever implementations inside a handler -- and `OnClose` which is the same as `OnConnectionClose(myFunc)` and `defer myFunc()` [*](https://github.com/hidevopsio/iris/commit/6898c2f755a0e22aa42e3b1799e29c857777a6f9)

This release contains minor grammar and typo fixes and more meaningful [godoc](https://godoc.org/github.com/hidevopsio/iris) code comments too. 
...
- **view/pug**: update vendor for Pug (Jade) parser and add [Iris + Pug examples](https://github.com/hidevopsio/iris/tree/master/_examples#view) via [this commit](https://github.com/hidevopsio/iris/commit/e0171cbed69efecba199ef547aa5e7063e18b27a), relative to [issue #1003](https://github.com/hidevopsio/iris/issues/1003) opened by [@DjLeChuck](https://github.com/DjLeChuck)
- **middleware/logger**: new configuration field, defaults to false: `Query bool`, if true prints the full path, including the URL query as requested at [issue #1017](https://github.com/hidevopsio/iris/issues/1017) by [@andr33z](https://github.com/andr33z). Example [here](https://github.com/hidevopsio/iris/blob/master/_examples/http_request/request-logger/main.go#L21). Implemented by [this commit](https://github.com/hidevopsio/iris/commit/a7364876e0d1b8bd60acf94f17f6d1341b16c617)
- **cookies**: some minor but helpful additions, like `CookieOption` relative to [issue #1018](https://github.com/hidevopsio/iris/issues/1018) asked by [@dibyendu](https://github.com/dibyendu). [Cookies examples added](https://github.com/hidevopsio/iris/tree/master/_examples/cookies) too. Implemented by [this commit](https://github.com/hidevopsio/iris/commit/574414a64ed3d8736c836d476e6304d915f4a511)
- **cookies**: ability to set custom cookie encoders to encode the cookie's value before sent by `context#SetCookie` and `context#SetCookieKV` and cookie decoders to decode the cookie's value when retrieving from `context#GetCookie`. That was the second and final part relative to a community's question at: [issue #1018](https://github.com/hidevopsio/iris/issues/1018). Implemented by [this commit](https://github.com/hidevopsio/iris/commit/f708c6098faec7c4e2232c791380cdff7a26960b)
- **fix**: [issue #1020](https://github.com/hidevopsio/iris/issues/1020) via [this commit](https://github.com/hidevopsio/iris/commit/3d30ccef05703246b716a14dda14d2f28294dbd2), redis database stores the int as float64, don't change that native behavior, just grab it nicely.

## Translations (2)

- [README_PT_BR.md](README_PT_BR.md) for Brazilian Portuguese language via [this PR](https://github.com/hidevopsio/iris/pull/1008) thanks to [@gschri](https://github.com/gschri)
- [README_JPN.md](README_JPN.md) for Japanese language via [this PR](https://github.com/hidevopsio/iris/pull/1015) thanks to [@tkhkokd](https://github.com/tkhkokd).

Thank you both for your contribution. We all looking forward for the HISTORY translations as well!!!

# Mo, 21 May 2018 | v10.6.5

First of all, special thanks to [@haritsfahreza](https://github.com/haritsfahreza) for translating the entire Iris' README page & Changelogs to the Bahasa Indonesia language via PR: [#1000](https://github.com/hidevopsio/iris/pull/1000)!
...
Don't forget to [star](https://github.com/hidevopsio/iris/stargazers) the Iris' github repository whenever you can and spread the world about its potentials!
...
- [fix issue 995](https://github.com/hidevopsio/iris/commit/62457279f41a1f157869a19ef35fb5198694fddb)
- [fix issue 996](https://github.com/hidevopsio/iris/commit/a11bb5619ab6b007dce15da9984a78d88cd38956)
...
- Websocket: fix https://github.com/hidevopsio/iris/issues/991

# Tu, 01 May 2018 | v10.6.2

- Websocket: added OnPong to Connection via PR: https://github.com/hidevopsio/iris/pull/988
- Websocket: `OnError` accepts a `func(error)` now instead of `func(string)`, as requested at: https://github.com/hidevopsio/iris/issues/987
...
- Fix open redirect by @wozz via PR: https://github.com/hidevopsio/iris/pull/972.
- Fix when destroy session can't remove cookie in subdomain by @Chengyumeng via PR: https://github.com/hidevopsio/iris/pull/964.
- Add `OnDestroy(sid string)` on sessions for registering a listener when a session is destroyed with commit: https://github.com/hidevopsio/iris/commit/d17d7fecbe4937476d00af7fda1c138c1ac6f34d.
- Finally, sessions are in full-sync with the registered database now. That required a lot of internal code changed but **zero code change requirements by your side**. We kept only `badger` and `redis` as the back-end built'n supported sessions storages, they are enough. Made with commit: https://github.com/hidevopsio/iris/commit/f2c3a5f0cef62099fd4d77c5ccb14f654ddbfb5c relative to many issues that you've requested it.

# Sa, 24 March 2018 | v10.5.0

### New

Add new client cache (helpers) middlewares for even faster static file servers. Read more [there](https://github.com/hidevopsio/iris/pull/935).
...
The proposal was made by @jefurry at https://github.com/hidevopsio/iris/issues/937.
...
- fix high path cleaning between `{}` (we already escape those contents at the [interpreter](macro/interpreter) level but some symbols are still removed by the higher-level api builder) , i.e `\\` from the string's macro function `regex` contents as reported at [927](https://github.com/hidevopsio/iris/issues/927) by [commit e85b113476eeefffbc7823297cc63cd152ebddfd](https://github.com/hidevopsio/iris/commit/e85b113476eeefffbc7823297cc63cd152ebddfd)
...
- Fix trailing slash from POST method request redirection as reported at: https://github.com/hidevopsio/iris/issues/921 via https://github.com/hidevopsio/iris/commit/dc589d9135295b4d080a9a91e942aacbfe5d56c5

-  Add examples for read using custom decoder per type, read using custom decoder via `iris#UnmarshalerFunc` and to complete it add an example for the `context#ReadXML`, you can find them [here](https://github.com/hidevopsio/iris/tree/master/_examples#how-to-read-from-contextrequest-httprequest)via https://github.com/hidevopsio/iris/commit/78cd8e5f677fe3ff2c863c5bea7d1c161bf4c31e.

- Add one more example for custom router macro functions, relative to https://github.com/hidevopsio/iris/issues/918, you can find it [there](https://github.com/hidevopsio/iris/blob/master/_examples/routing/dynamic-path/main.go#L144-L158), via https://github.com/hidevopsio/iris/commit/a7690c71927cbf3aa876592fab94f04cada91b72

- Add wrappers for `Pongo`'s `AsValue()` and `AsSaveValue()` by @neenar via PR: https://github.com/hidevopsio/iris/pull/913

- Remove unnecessary reflection usage on `context#UnmarshalBody` via https://github.com/hidevopsio/iris/commit/4b9e41458b62035ea4933789c0a132c3ef2a90cc
...
As correctly pointed out by @likakuli at https://github.com/hidevopsio/iris/issues/901, the old `Done` registered
...
- Multi-Level subdomain redirect helper, you can find an example [here](https://github.com/hidevopsio/iris/blob/master/_examples/subdomains/redirect/main.go)
- Cache middleware which makes use of the `304` status code, request fires from client to server but server respond with a status code, client is responsible to render the cached, you can find an example [here](https://github.com/hidevopsio/iris/blob/master/_examples/cache/client-side/main.go)
...
- [don't force-set content type on gzip response writer's WriteString and Writef if already there](https://github.com/hidevopsio/iris/commit/af79aad11932f1a4fcbf7ebe28274b96675d0000)
- [new: add websocket/Connection#IsJoined](https://github.com/hidevopsio/iris/commit/cb9e30948c8f1dd099f5168218d110765989992e)
- [fix #897](https://github.com/hidevopsio/iris/commit/21cb572b638e82711910745cfae3c52d836f01f9)
- [add context#StatusCodeNotSuccessful variable for customize even the rfc2616-sec10](https://github.com/hidevopsio/iris/commit/c56b7a3f04d953a264dfff15dadd2b4407d62a6f)
- [fix example comment on routing/dynamic-path/main.go#L101](https://github.com/hidevopsio/iris/commit/0fbf1d45f7893cb1393759b7362444f3d381d182)
- [new: Cache Middleware `iris.Cache304`](https://github.com/hidevopsio/iris/commit/1722355870174cecbc12f7beff8514b058b3b912)
- [fix comment on csrf example](https://github.com/hidevopsio/iris/commit/a39e3d7d6cf528e51e6c7e32a884a8d9f2fadc0b)
- [un-default the Configuration.RemoteAddrHeaders](https://github.com/hidevopsio/iris/commit/47108dc5a147a8b23de61bef86fe9327f0781396)
- [add vscode extension link and badge](https://github.com/hidevopsio/iris/commit/6f594c0a7c641cc98bd683163fffbf5fa5fc8de6)
...
- [new: Support multi-level subdomains redirect](https://github.com/hidevopsio/iris/commit/12d7df113e611a75088c2a72774dab749d2c7685).
...
- fix a single one cache handler didn't work across multiple route handlers at the same time https://github.com/hidevopsio/iris/pull/852, as reported at https://github.com/hidevopsio/iris/issues/850
- merge PR https://github.com/hidevopsio/iris/pull/862
- do not allow concurrent access to the `ExecuteWriter -> Load` when `view#Engine##Reload` was true, as requested at https://github.com/hidevopsio/iris/issues/872
...
1. The Chinese README_ZH.md and HISTORY_ZH.md was translated by @Zeno-Code via https://github.com/hidevopsio/iris/pull/858
2. New Russian README_RU.md translations by @merrydii via https://github.com/hidevopsio/iris/pull/857
3. New Greek README_GR.md and HISTORY_GR.md translations via https://github.com/hidevopsio/iris/commit/8c4e17c2a5433c36c148a51a945c4dc35fbe502a#diff-74b06c740d860f847e7b577ad58ddde0 and https://github.com/hidevopsio/iris/commit/bb5a81c540b34eaf5c6c8e993f644a0e66a78fb8
...
> The new version adds 75 plus new commits, the PR is located [here](https://github.com/hidevopsio/iris/pull/849) read the internal changes if you are developing a web framework based on Iris. Why 9 was skipped? Because.
...
| NEW | OLD |
| -----------|-------------|
| [Hello world](_examples/mvc/hello-world/main.go) | [OLD Hello world](https://github.com/hidevopsio/iris/blob/v8/_examples/mvc/hello-world/main.go) |
| [Session Controller](_examples/mvc/session-controller/main.go) | [OLD Session Controller](https://github.com/hidevopsio/iris/blob/v8/_examples/mvc/session-controller/main.go) |
| [Overview - Plus Repository and Service layers](_examples/mvc/overview) | [OLD Overview - Plus Repository and Service layers](https://github.com/hidevopsio/iris/tree/v8/_examples/mvc/overview) |
| [Login showcase - Plus Repository and Service layers](_examples/mvc/login) | [OLD Login showcase - Plus Repository and Service layers](https://github.com/hidevopsio/iris/tree/v8/_examples/mvc/login) |
